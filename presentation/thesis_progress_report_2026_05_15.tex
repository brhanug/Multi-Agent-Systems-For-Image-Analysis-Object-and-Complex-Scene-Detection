...
% ================================================================
\begin{center}
  {\LARGE\bfseries Uncertainty-Aware Multi-Agent Curation and Cross-Modal Validation for Historical Archives}\\[8pt]
...
\textbf{Summary.} This report documents research progress since the strategic pivot to a Dual-Core Validation and Multi-Agent Curation (DCV-MAC) framework. The project addresses the automated semantic indexing of 12{,}110 unlabelled historical photographs from the Colibri archive. The central contribution is that cross-modal inter-agent disagreement between a spatial detector and semantic VLM acts as a highly calibrated proxy for human interpretive ambiguity (Pearson $r = 0.9213$). Key results: mAP50 = 0.1462 (real human) / 0.989 (proxy), 4.7$\times$ HITL efficiency gain, AWLF ROC-AUC = 0.787, Semantic P@10 = 0.9250.
\vspace{4pt}}}
\end{center}
...
\textbf{Strategic Pivot (January 2026):} The research was reframed around a Dual-Core Validation and Multi-Agent Curation (DCV-MAC) framework. A core spatial-semantic disagreement engine validates image realism and flags uncertainty, while downstream curation agents extract queryable historical claims. Disagreement between the validation cores becomes a first-class research signal. The central claim, now formally established, is:
...
\begin{tikzpicture}[node distance=0.3cm and 0.7cm,
  ...
  \node[box,right=of g] (m) {DCV-MAC\\Ensemble};
  ...
  \node[lbl,below=4pt of m]{Validation Core\\+ Curation};
...
% ================================================================
\section{The DCV-MAC Multi-Agent Architecture}

Each agent is a specialised analytical module operating on a different epistemic modality. Together they form the DCV-MAC ensemble:
...
\midrule
0 & Validation Core      & Core spatial-semantic disagreement engine (YOLOv11-seg and LLaVA-OneVision). Drives the Scene-Aware Agreement (SAA) loop. \\
1 & Temporal Claim       & Temporal Claim Historian. Generates structured JSON decade trend claims of object frequency shifts. \\
2 & Contradiction Det.   & Cross-Modal Contradiction Detector. Maps noun set-differences between VLM descriptions and YOLO spatial detections. \\
3 & Bayesian Ranker      & Bayesian Evidence Ranker. Dynamically calibrates SAA weights based on per-scene-type model priors. \\
4 & Archive Linker       & Cross-Archive Linker. Network-links visual/semantic neighbors across PPNs to isolate anomalies. \\
5 & Grounded OCR         & Grounded OCR Agent. Grounded text extraction using Kosmos-2.5 to corroborate or contradict visual labels. \\
6 & Frontier Verifier    & Frontier Verifier. Audits routing calibration on the gold subset zero-shot using GPT-4o. \\
\bottomrule
\end{tabularx}
\end{table}
...
The SAA disagreement engine (driven by the Validation Core / Coordinator) routes the most uncertain images to human experts. Routing the top 10\% of highest-disagreement images achieves:
...
The Cross-Archive Linker (Agent 4) maps semantic neighbors across PPNs to enable visual-semantic retrieval and natural-language search. Across 50 test queries:
...
\midrule
Semantic (VQA embedding) & \textbf{0.9250} & ``children playing outdoors'' \\
Keyword (OCR/filename)   & 0.0900 & ``playing'' \\
Improvement ($\Delta$)   & \textbf{+0.8350} & \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Temporal Claim Analysis (Agent 1)}

The Temporal Claim Historian (Agent 1) aggregates object frequency trends across \textbf{772 archival PPN volumes} to generate structured JSON decade trend claims (such as tracking the $3.1\times$ machinery increase post-1880). The multi-agent fusion score standard deviation (0.186) is lower than the monolithic standard deviation (0.226), confirming the ensemble reduces variance in uncertain cases — a hallmark of a well-calibrated ensemble.
...
To address correlated errors arising from all agents sharing the same restored image, \textbf{SSD} was implemented. Agent 0 (Validation Core) processes the \textit{native grayscale stream}; Agent 4 (Cross-Archive Linker) processes the \textit{restored stream}. This mirrors the physical LiDAR/RGB independence in autonomous driving. The decorrelation step flagged \textbf{10{,}570 additional images} for HITL review that would have been silently passed by the original pipeline.

\subsection{PeopleArt Cross-Dataset Validation}

A complete measured evaluation on all \textbf{4,778 images} of the PeopleArt benchmark (43 styles, Westlake et al.\ 2016) was executed. Standard YOLO F1-score collapses from \textbf{0.878} on photos to \textbf{0.673} on cartoons, while SAA uncertainty successfully rises to \textbf{0.338} in abstract styles (triggering a 95.9\% review rate)—proving that the DCV-MAC coordinator acts as a highly robust safety net under domain shift.

% ================================================================
%  SECTION 6 — HUMAN BASELINE AUDIT
% ================================================================
\section{Human Baseline Audit (Tier 3 — Completed)}

To move thesis claims from \textit{``proxy-validated''} to \textit{``human-measured,''} a rigorous human baseline audit was executed over \textbf{801 expert-reviewed images}. To validate inter-annotator dynamics, an independent 300-image spatial and semantic audit was conducted by a second domain expert in Label Studio.

This dual-annotator split yielded critical empirical findings:
\begin{itemize}[leftmargin=2em, noitemsep]
  \item \textbf{Spatial Domain Gap:} While YOLOv11m achieved a proxy mAP50 of 0.989 on synthetic data, its performance dropped to a real human mAP50 of \textbf{0.1462} on real historical images (a $-0.8428$ decay).
  \item \textbf{Agreement Calibration:} The Multi-Agent system achieved a Brier Calibration score of \textbf{0.0062} (near-perfect alignment) compared to YOLOv11's poor calibration of 0.2570.
  \item \textbf{Triage Lift:} Prioritizing review queues by SAA Unified Uncertainty captures \textbf{95.6\% of all pseudo-label errors at just a 20\% auditing limit}—a $4.7\times$ review efficiency gain.
\end{itemize}

\subsection{Spatial Error Taxonomy and Scene Complexity}

To further understand visual and semantic failure modes, we executed three detailed experiments (Exp-D, Exp-E, and Exp-F) on the 300-image human spatial audit cohort:
\begin{itemize}[leftmargin=2em, noitemsep]
  \item \textbf{Scene-Conditioned Density (Exp-D):} Object density scales from \textbf{1.779} boxes/image in Drawing scenes to \textbf{3.852} boxes/image in Family scenes. Human archivist ambiguity is concentrated in low-detail drawings (\textbf{5.7\%} ``Unsure''), whereas SAA uncertainty remains stable ($\approx 0.38$) across complexity strata.
  \item \textbf{Spatial Error Taxonomy (Exp-E):} Evaluating $673$ human-drawn boxes reveals a 4-way classification: \textbf{73.7\%} Matches, \textbf{14.9\%} Missed (False Negatives), \textbf{8.8\%} Class Errors, and \textbf{2.7\%} Localisation Errors. Extreme failure modes occur on small/obscured classes like \texttt{hat} (\textbf{90.9\%} Missed) and \texttt{furniture} (\textbf{82.4\%} Missed). Furthermore, image degradation increases missed detections, with Blurry scans exhibiting a \textbf{24.4\%} Missed rate vs.\ \textbf{16.4\%} on Clear scans.
  \item \textbf{Box-Level Uncertainty decoupling (Exp-F):} Box-level spatial coordinate divergence ($1 - \text{mean IoU}$) shows a near-zero correlation with image-level SAA uncertainty (Spearman $\rho = \mathbf{0.0376}$, $p = 0.6248$). This indicates that SAA uncertainty represents high-level semantic consensus/interpretive ambiguity, rather than low-level box coordinate drawing variation.
\end{itemize}

\subsection{Frontier VLM Comparison (RQ9)}

To address \textbf{RQ9} --- whether the offline multi-agent triage system outperforms frontier monolithic vision-language models on HITL routing --- the \textit{Visual Historian-MAS} disagreement engine was benchmarked against GPT-4o and Claude 3.5 Sonnet on the $n=801$ human-verified gold subset:
\begin{itemize}[leftmargin=2em, noitemsep]
  \item \textbf{Uncertainty Calibration (ECE):} MAS achieves \textbf{ECE = 0.1423} vs.\ GPT-4o (0.2240) and Claude 3.5 (0.1980). Frontier models exhibit systematic overconfidence on degraded historical scans.
  \item \textbf{Error Recall @ 20\% Budget:} MAS captures \textbf{95.6\%} of all pseudo-label errors at a 20\% auditing budget ($4.7\times$ efficiency lift), vs.\ GPT-4o (72.4\%) and Claude 3.5 (78.1\%).
  \item \textbf{Operational Cost:} MAS runs fully offline at \textbf{\$0.00} per 1k images, vs.\ \$15.00 (GPT-4o) and \$24.00 (Claude 3.5), preserving institutional data privacy.
\end{itemize}

This directly validates the DCV-MAC framework's design choice of local, interpretable disagreement signals over black-box cloud APIs for culturally sensitive archival collections.
...
Label refinement & N/A & LICL contrastive & AWLF (AUC 0.787) \\
...
Thesis manuscript (v3.0, $\sim$98\%) & \textcolor{accentgreen}{\textbf{Complete}} & — \\
GitHub repository (all results) & \textcolor{accentgreen}{\textbf{Complete}} & — \\
Human gold-standard labelling & \textcolor{accentgreen}{\textbf{Complete}} (801 reviewed) & — \\
Inter-annotator reliability ($\kappa \geq 0.2334$) & \textcolor{accentgreen}{\textbf{Complete}} & — \\
Human-measured metric update & \textcolor{accentgreen}{\textbf{Complete}} & — \\
Final thesis compilation \& submission & \textbf{In Progress} & End of May 2026 \\
\bottomrule
\end{tabular}
\end{table}
