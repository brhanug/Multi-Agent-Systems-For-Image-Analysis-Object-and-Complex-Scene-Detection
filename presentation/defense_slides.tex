...
\node[tierbox1] (t1) {\textbf{TIER 1: Validation Core}\\YOLOv11+SAM (Grounding)\\SigLIP (Scene Prior)\\LLaVA-OneVision (VQA)\\Kosmos-2.5 (OCR)\\$\rightarrow$ \textbf{Calculates $SAA_i$ \& $U_{\text{SAA},i}$}\\{\scriptsize (two distinct signal sets, 2.6)}};
...
\node[coordbox, right=of t2] (coord) {\textbf{Dual-LLM Coordinator}\\Frontier-only\\Gemini proposes\\Claude audits};
...
\vspace{0.5mm}
\tiny\textit{Frontier-only, no local equivalent: a same-model local variant was tested and dropped after catching 0/25 self-contradictions (Slide 2.3/A.5); the frontier independent-model pair's own advantage remains unverified.}
\normalsize
\vspace{0.5mm}
...
\end{block}
\begin{block}{Why Frontier-Only: A Local Design We Dropped}
\scriptsize An earlier design reused Tier 1's LLaVA-OneVision locally (free) as both Primary and Critic, re-prompted. A real test found this catches \textbf{0/25} self-contradictions -- same weights, same beliefs, regardless of prompt. Dropped; no local second opinion exists (A.5).
\end{block}

\column{0.49\textwidth}
\begin{block}{Tier 3: Frontier-Only Selective Escalation}
Triggered \emph{only} for the small $\sigma > \tau$ fraction Tiers 1--2 cannot resolve. Paid frontier APIs (Gemini 2.5 Flash proposes / Claude 3.5 audits) -- two structurally independent models, cheaper and faster than routing straight to a human. This independence advantage is itself unverified (API credit), but is a real, distinct claim the dropped local design never had (Backup A.5).
\end{block}
\begin{block}{Why an Escalation Ladder, Not One Model}
Cost/latency increase per tier (free deterministic $\to$ free heuristic $\to$ paid API $\to$ human); each tier sees only what the last could not resolve. $U_{\text{SAA}}$ decides how far an image travels -- it is what keeps the expensive tiers rare.
\end{block}
\end{columns}
\end{frame}
...
\vskip 0.1cm
\tiny \textit{Teacher-stage training progress (illustrative). The 0.989 vs.\ 0.1462 collapse is a separate final evaluation, not these curves' own endpoint.}
\end{columns}
\end{frame}
...
Threshold $\hat{\lambda} = 0.5370$ at target $\alpha = 0.05$. Evaluating on the same-cohort held-out test split ($n_{\text{test}}=401$) appears to satisfy $\mathcal{R}(\hat{\lambda}) = \mathbf{0.0075} \le \alpha = 0.05$.
\begin{itemize}
...
\begin{frame}{A.5 Why Local Tier 3 Was Dropped}
\scriptsize
\begin{columns}[T]
\column{0.50\textwidth}
...
\begin{itemize}
  \item \textbf{Local} (LLaVA-OneVision, $n=25$): \textbf{0/25} $\rightarrow$ dropped.
  \item \textbf{Stronger approx.} (Gemini Flash-Lite, $n=10$): \textbf{1/10}, still same-model.
\end{itemize}
\end{block}

\column{0.46\textwidth}
\begin{block}{Why This Was Enough to Drop It}
\scriptsize A stronger model catches slightly more of its own errors under re-prompting, but even that is same-model self-review, not the cross-model independence (Gemini proposes, Claude audits) the frontier design relies on. With zero verified value locally, Tier 3 is now frontier-only: the ambiguous residual either pays for a genuine second opinion, or goes straight to a human.
\end{block}
\begin{block}{Frontier's Own Test Remains Unrun}
\scriptsize The genuine cross-model independence test (Anthropic key lacked credit) is unrun, not falsified. Disjoint weights can't share a blind spot is a real, distinct argument the dropped local design never had -- but it is still an argument, not yet a measurement.
\end{block}
\end{columns}
\end{frame}
...
\node[stagebox] (s1) {\textbf{Deployed: SAA}\\$w{=}[0.6,0.2,0.2]$\\Heuristic, all results};
\node[stagebox, right=of s1] (s2) {\textbf{Interpretability: Shapley}\\$\phi{=}[0.33,0.28,0.20,0.19]$\\Exact attribution, not deployed};

\draw[arrow, dashed] (s1) -- (s2) node[midway, above, font=\tiny] {separate study};
\draw[arrow] (s1.south) -- ++(0,-0.35) node[below, font=\tiny, align=center] {Platt calibration\\applied here directly\\$\mathbf{\text{ECE} < 0.001}$};
\end{tikzpicture}
...
\scriptsize A supplementary interpretability study only -- quantifies contribution to a coalitional objective, not independent causal correctness. No result elsewhere uses these Shapley weights.
\end{alertblock}
...
\vskip 0.1cm
\tiny \textit{Downstream performance of Shapley-weighted fusion vs.\ equal-weight and VLM-only -- not a plot of the $\phi_i$ values themselves (those are at left).}
\end{columns}
\end{frame}
