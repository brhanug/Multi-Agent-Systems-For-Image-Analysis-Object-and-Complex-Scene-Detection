...
\midrule
None (Full 6-Agent System) & 0.1555 & 8.41\% \\
\midrule
Remove Agent 0 (CV Pipeline) & 0.2800 & 24.50\% \\
Remove Agent 1 (Temporal) & 0.1420 & 7.10\% \\
Remove Agent 2 (Retrieval) & 0.1450 & 7.30\% \\
Remove Agent 3 (Critic) & 0.1380 & 6.80\% \\
Remove Agent 4 (Demographic) & 0.1490 & 7.80\% \\
Remove Agent 5 (Geospatial) & 0.1510 & 8.10\% \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:macro_ablation} proves that \textbf{Agent 0} is the foundational anchor of the system; removing it causes uncertainty to spike massively, flagging nearly a quarter of the archive for manual review. However, Agents 1-5 all contribute to raising necessary epistemic doubt (increasing $\sigma$ from $\sim 0.14$ up to $0.1555$). This demonstrates that the analytical agents are successfully injecting critical historiographic nuance that the raw CV pipeline misses, routing the most ambiguous scenes to human experts.
