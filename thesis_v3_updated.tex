...
% --------------------------------------------------------------------------
\section{Architectural Separation: Validation vs.\ Enrichment Agents}
\label{sec:e3_decoupling}

\subsection{Motivation}

The original multi-agent coordinator (pre-E3) blended all six agent signals into a single \texttt{overall\_realism\_score}:
...
\subsection{E3 Architectural Fix}

We refactor \texttt{run\_multi\_agent\_validation.py} to implement strict role separation:

\begin{itemize}
  \item \textbf{Validation agents} (object, agreement, scene, VLM): contribute to \texttt{overall\_realism\_score} via renormalised equal-weight fusion. These agents all answer the same question: \textit{``Is this scene realistically indexed?''}
  \item \textbf{Enrichment agents} (document, restoration): output as \texttt{enrichment\_document\_score} and \texttt{enrichment\_restoration\_score} metadata fields only. They contribute to the archive's qualitative value (OCR searchability, preprocessing quality) but \textit{not} to the realism scalar.
  \item \textbf{Uncertainty} is now computed exclusively from the 4 validation agent scores (population standard deviation), ensuring it reflects inter-agent disagreement on the shared validation task.
\end{itemize}
