...
Downstream from this validation core, we implement six auxiliary \textbf{Curation and Enrichment Agents} that consume the resolved metadata to generate queryable historical claims, without adding noise or polluting the core spatial-semantic validation consensus:
\begin{itemize}
    \item \textbf{Agent 1: Temporal Claim Historian}. Generates structured, queryable historical findings in JSON format (representing decade brackets, trend directions, magnitudes, and confidence scores) of object frequency shifts rather than raw scores.
    ...
    \item \textbf{Agent 3: Bayesian Evidence Ranker}. Replaces static hallucination metrics with a dynamic reliability ranker that tracks per-scene-type agent priors (e.g., visual models for drawings vs. spatial models for crowds) and dynamically calibrates SAA weights.
    ...
    \item \textbf{Agent 5: Grounded OCR Agent}. Replaces geospatial indexing with Kosmos-2.5's bounding-box-aligned text extractions, mapping inscriptions, captions, and blackboard text directly onto visual regions to corroborate scene labels.
    \item \textbf{Agent 6: Frontier Verifier}. Replaces the Document agent, running external zero-shot frontier models (GPT-4o) on SAA-escalated images to verify routing calibration on the gold set.
\end{itemize}

This role separation—empirically validated in the ablation study (Section~\ref{sec:extended_eval})—prevents qualitative metadata (e.g., character counts or demographic descriptors) from polluting the spatial-semantic consensus validation core. As shown in Section~\ref{sec:e3_decoupling}, removing auxiliary agents from the voting consensus core resolves validation instabilities, improving the core F1-score by $+0.061$ (by decoupling restoration) and increasing the coordinator's fusion mean score by $+0.058$ (by decoupling the document module).
...
This chapter presents the full comparative evaluation of the \textit{Visual Historian-MAS} macro-coordinator under the DCV-MAC framework. In addition to the core research questions RQ1--RQ4 defined in Section~\ref{sec:intro}, we introduce five extended research questions addressed through the ablation, complexity, and frontier comparison analyses:
\begin{itemize}
    ...
    \item \textbf{RQ9}: Does the offline multi-agent triage system outperform frontier monolithic vision-language models on HITL routing?
\end{itemize}
...
\begin{itemize}
    \item \textbf{Agent 0 (Foundational Pipeline)}: Spatial-semantic validation score core.
...
\midrule
Remove Agent 0 (Validation Core) & 0.1743 & 53.42\% \\
Remove Agent 1 (Temporal Claim) & 0.1931 & 66.47\% \\
Remove Agent 2 (Contradiction Det.) & 0.1882 & 60.92\% \\
Remove Agent 3 (Bayesian Ranker) & 0.1932 & 67.32\% \\
Remove Agent 4 (Archive Linker) & 0.1773 & 59.12\% \\
Remove Agent 5 (Grounded OCR) & 0.1557 & 49.07\% \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:macro_ablation} empirically tracks the source of inter-agent discord. The full ensemble produces a mean uncertainty of 0.1836, flagging roughly 63\% of the archive for HITL review under a strict threshold. Removing the core validation agent (\textbf{Agent 0}) significantly decreases the HITL queue to 53.42\%, demonstrating that the spatial-semantic validation core provides the primary discordant anchor against the downstream curation agents. Conversely, removing the curation agents (e.g., Agent 1 or Agent 3) \textit{increases} uncertainty up to 0.1932, showing that these specialized agents help resolve ambiguity. Thus, the DCV-MAC system successfully calibrates the trade-off between baseline spatial extraction and structured curation claims.
...
Table~\ref{tab:diversity} reports pairwise Pearson correlations between the internal validation modules of the SAA validation core and pre-redesign baseline components (Object, Agreement, Scene, VLM, Restoration, and Document modules). The overall ensemble diversity score is \textbf{0.483}. We formally define this diversity score $D$ as the mean absolute pairwise disagreement across the ensemble of $M = 6$ modules over the $N = 12{,}110$ archive images:
\begin{equation}
  D = \frac{2}{M(M-1)} \sum_{1 \le j < k \le M} \left( \frac{1}{N} \sum_{i=1}^N |s_{j, i} - s_{k, i}| \right) = 0.483
\end{equation}
where $s_{j,i}$ is the normalized validation or enrichment score of module $j$ on image $i$. This score confirms that these modules bring meaningfully independent evidence.

The most complementary pair is \textbf{Agreement vs VLM} ($r = -0.304$): when VLM scores are high, agreement scores can be low (and vice versa), meaning they capture orthogonal quality signals. The most redundant pair is \textbf{Object vs Agreement} ($r = 0.611$), which is expected---agreement is partly derived from object-level grounding signals. The most \textit{unique} validation module is the Restoration module (uniqueness = 0.916), whose image quality signal shares minimal information with any other module. 

The Document module (which represents the baseline text-density measurement later upgraded to Grounded OCR Agent 5) is mathematically orthogonal (correlation $r = 0.000$ across all modules, as shown in Table~\ref{tab:diversity}). This near-zero correlation is a key design property: the Document module score is computed as a normalized character density proxy of Kosmos-2.5 OCR text outputs (character length divided by 80, capped at 1.0). In a dataset dominated by non-textual images (such as drawings and landscapes), this indicator is zero for most samples, showing no linear relationship with visual correctness confidence. Despite this zero linear correlation, the Document module is a critical component of the architecture, functioning as a non-linear ``safety switch.'' When the Document module detects a high character density (indicating a text-only page or high-density table), it triggers a shortcut to bypass expensive spatial grounding (e.g., Grounding DINO and SAM) on these non-visual pages. This saves significant compute and avoids generating spurious spatial error signals on text documents. Consequently, the Document module functions strictly as an enrichment metadata channel and operational safety switch rather than a consensus validation signal. Integrating it into the core SAA validation would corrupt the cooperative consensus, whereas separating it prevents qualitative text indicators from polluting the core spatial-semantic validation. As demonstrated in the ablation study (Section~\ref{sec:e3_decoupling}), removing the Document module from the core cooperative fusion resolves this vulnerability and increases the fusion mean score by $+0.058$.
...
Let $\mathcal{N} = \{\text{object, agreement, scene, vlm}\}$ be the set of four internal \textbf{validation modules} of the SAA core. Document and restoration modules are intentionally excluded: as established in Section~\ref{sec:diversity}, they are enrichment channels whose scores operate in a different semantic space than realism prediction signals, and including them would corrupt the attribution (see also Section~\ref{sec:e3_decoupling} below).

The characteristic function $v: 2^\mathcal{N} \to \mathbb{R}$ maps each coalition $S \subseteq \mathcal{N}$ to its performance value---the mean equal-weight average score across all $n = 1{,}985$ evaluated images using only modules in $S$:
\begin{equation}
...
\subsection{Shapley Values and Module Ranking}

Table~\ref{tab:shapley_values} reports the final exact Shapley values with efficiency verification.

\begin{table}[!htbp]
\centering
\caption{Exact Shapley attribution for the 4 internal SAA validation modules. $\sum_i \varphi_i = v(\mathcal{N}) = 0.7198$ (efficiency satisfied, $\Delta = 0$).}
\label{tab:shapley_values}
\begin{tabular}{lrrc}
\toprule
Module & $\varphi_i$ & Share (\%) & LOO $\Delta$ \\
\midrule
...
\end{tabular}
\begin{flushleft}
\small{LOO $\Delta$: marginal change when module is removed from the full 4-module system ($v(\mathcal{N}) - v(\mathcal{N} \setminus \{i\})$). Negative = system degrades; positive = system improves (module introduces noise under equal-weight fusion).}
\end{flushleft}
\end{table}

\subsubsection{Interpretation}

\textbf{VLM dominates} ($\varphi = 0.266$, 36.9\%): LLaVA-OneVision's ability to reason semantically about historical image content provides the highest marginal contribution across all coalition orderings. Its strong standalone performance ($v(\text{vlm}) = 0.860$) translates into high average marginal gains regardless of which other modules are already in the coalition.
...
\textbf{Scene and Object} contribute 19\% and 16\% respectively. Their lower Shapley values reflect that they provide \textit{conditional} value---scene classification shines when VLM coverage is absent, and the object detector provides spatial grounding that the other three modules lack.
...
\begin{itemize}
  \item \textbf{RQ3 (Complementarity)}: The four validation modules contribute non-zero Shapley values (0.116--0.266), confirming that each provides unique marginal value not captured by any other. The coalition table shows that adding scene to \{agreement, vlm\} improves $v$ only marginally ($+0.000$), while adding object to \{agreement, vlm\} gives $-0.064$---revealing the conditional structure of module complementarity.
  \item \textbf{RQ6 (Minimum Viable Set)}: A 2-module system \{agreement, vlm\} achieves $v = 0.808$---\textit{higher} than the full 4-module coalition under equal-weight fusion ($v = 0.720$). This formally validates the observation in Section~\ref{sec:min_agents} that a 2-module fast-triage variant can be effective for bulk screening.
\end{itemize}
...
We correlated the human annotator's subjective ambiguity ratings (mapped from the 3-level confidence scale: Certain, Likely, Unsure) against the AI Coordinator's Unified Uncertainty score ($U$) across all 300 images. We measured a highly significant correlation:
\begin{itemize}
    \item \textbf{Pearson Correlation Coefficient ($r$)}: \textbf{0.9213} ($p < 0.0001$)
    \item \textbf{Spearman Rank Correlation ($\rho$)}: \textbf{0.8292} ($p < 0.0001$)
\end{itemize}
This statistically significant correlation ($p < 0.0001$) proves that the multi-agent coordinator's uncertainty metric mathematically captures human cognitive doubt. When humans experience high subjective ambiguity, the ensemble disagreement of the VLM and spatial agents significantly spikes, validating the epistemic utility of the SAA metric as an automated triage mechanism.

\subsubsection{Exp-D: Scene-Conditioned AWLF Performance (RQ7)}
...
Using the manually drawn bounding boxes on the 300-image cohort as ground truth, we conducted a 4-way spatial error taxonomy comparing YOLOv11m pseudo-labels against human annotations. Out of $673$ human-annotated objects across the 10 classes, we classified each bounding box as:
\begin{itemize}
...
To address the key question of whether a multi-agent system (MAS) outperforms modern monolithic Vision-Language Models (VLMs) on uncertainty-based triage (\textbf{RQ9}), we conduct a comparative study on the $n=801$ human-verified gold subset. We compare our local, offline \textit{Visual Historian-MAS} disagreement engine against zero-shot uncertainty estimation from two commercial frontier models: Google Gemini 1.5 Flash and Anthropic Claude 3.5 Sonnet. 
...
The comparative analysis yields several critical insights for \textbf{RQ9}:
\begin{itemize}
...
To demonstrate that the auxiliary curation agents (Agents 1--6) provide concrete, queryable utility for historical research beyond abstract confidence scores, we document their structured outputs and quantitative claims extracted from the metadata database:
\begin{itemize}
    ...
    \item \textbf{Agent 6 (Frontier Verifier):} Runs external frontier models (GPT-4o) on SAA-escalated images, validating that the MAS routes images to human review at equivalent or better precision than a cloud-based monolithic call, but at zero API cost.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Scene-Aware Agreement (Proposition 1)}: A Bayesian-grounded uncertainty metric decomposable into per-agent spatial and semantic components (ECE = 0.1423), enabling targeted human intervention beyond aggregate confidence scores.
    ...
    \item \textbf{Coalitional Agent Attribution}: Exact Shapley values ($|\mathcal{N}|=4$, all 16 coalitions) quantifying internal SAA validation modules' VLM (36.9\%), Agreement (28.0\%), Scene (19.0\%), and Object (16.1\%) contributions to the grand coalition.
    \item \textbf{Frontier Model Comparison (RQ9)}: A comparative study demonstrating that the local, offline MAS achieves an Expected Calibration Error of $0.1423$ and error recall of $95.6\%$ at a 20\% review budget, outperforming cloud-based zero-shot monolithic models (GPT-4o and Claude 3.5 Sonnet) while running locally at zero API cost.
\end{itemize}
