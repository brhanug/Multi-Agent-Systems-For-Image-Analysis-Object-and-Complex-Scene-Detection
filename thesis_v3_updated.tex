...
This chapter presents the full comparative evaluation of the \textit{Visual Historian-MAS} macro-coordinator under the DCV-MAC framework. In addition to the core research questions RQ1--RQ4 defined in Section~\ref{sec:intro}, we introduce four extended research questions addressed through the ablation and complexity analyses:
\begin{itemize}
...
Following the reframed DCV-MAC architecture, the core disagreement validation engine (Agent 0 spatial-semantic core) is evaluated and cross-referenced with five specialized downstream curation agents (Agents 1--5).
...
\caption{Pairwise agent independence and diversity correlation matrix across the DCV-MAC curation system, showing high complementarity.}
...
\midrule
None (Full Curation Ensemble) & 0.1836 & 62.94\% \\
\midrule
...
Table~\ref{tab:macro_ablation} empirically tracks the source of inter-agent discord. The full ensemble produces a mean uncertainty of 0.1836, flagging roughly 63\% of the archive for HITL review under a strict threshold. Removing \textbf{Agent 0} significantly decreases the HITL queue to 53.42\%, demonstrating that the core CV pipeline provides the primary discordant anchor against the downstream analytical agents. Conversely, removing the analytical agents (e.g., Agent 1 or Agent 3) \textit{increases} uncertainty up to 0.1932, showing that these specialized agents help resolve ambiguity. Thus, the DCV-MAC system successfully calibrates the trade-off between baseline spatial extraction and historiographic nuance.
