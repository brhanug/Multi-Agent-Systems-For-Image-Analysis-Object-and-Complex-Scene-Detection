...
% ==========================================================================
\begin{abstract}
The automated analysis of historical archives is fundamentally constrained by the irreducible semantic ambiguity of archival content. While modern single-model architectures, including Vision-Language Models (VLMs) and object detectors trained via knowledge distillation, achieve high predictive accuracy, they are structurally incapable of diagnosing their own epistemic uncertainty. We present \textit{Visual Historian-MAS}, a framework that quantifies epistemic uncertainty in historical image analysis through multi-agent disagreement, enabling principled human-in-the-loop curation at scale. By orchestrating a multi-modal ensemble---comprising spatial grounding (YOLO), semantic reasoning (LLaVA), and scene retrieval (CLIP) agents---our central contribution demonstrates that inter-agent disagreement is a highly calibrated predictor of semantic failure. Evaluated against an out-of-sample, human-verified evaluation subset ($n=801$), disagreement-based routing perfectly captures complex archival failures where a single spatial detector is blindly overconfident, maintaining a Triage Precision@10\% of 1.000. Furthermore, a learned Agreement-Weighted Label Filter (AWLF) achieves an out-of-sample ROC-AUC of 0.787 on expert annotations, massively outperforming single-model heuristic thresholding. Ultimately, this thesis reframes AI in digital humanities not as an infallible oracle, but as an uncertainty-aware consensus network that routes critical historical ambiguities directly to domain experts.
\end{abstract}
...
This chapter defines the architectural framework of the \textit{Visual Historian-MAS} platform. We model archival analysis as a cooperative system of specialized agents rather than a single linear predictor. 
 ...
 The architecture is framed as a \textbf{6-Agent System}. The comprehensive, multi-stage computer vision pipeline---now including generative restoration, instance segmentation (YOLOv11-seg + SAM), zero-shot scene classification (SigLIP), VLM reasoning (LLaVA-OneVision), and grounded OCR (Kosmos-2.5)---is encapsulated as a single foundational unit (\textbf{Agent 0}). Operating downstream from this foundation are five independent analytical agents, each providing specialized historiographic analysis:

\begin{itemize}
    \item \textbf{Agent 0: Visual Historian Pipeline (Upgraded)}. The core feature extraction mechanism (Restoration $\to$ YOLOv11-seg+SAM $\to$ LLaVA-OneVision $\to$ SigLIP $\to$ Kosmos-2.5), internally fused via SAA.
    ...
    \item \textbf{Agent 3: Hallucination Critic}. Cross-checks VQA answers against YOLOv11-seg spatial detections, flagging semantic conflicts.
    \item \textbf{Agent 4: Demographic Profiler}. Extracts human clustering (persons, children) and clothing details from actual fusion JSON to compute a Social Composition Score.
    \item \textbf{Agent 5: Geospatial Analyst}. Evaluates the intersection of buildings, nature, and scene types from real metadata to classify images on an urban/rural/institutional spectrum.
\end{itemize}

The architecture comprises a \textbf{four-agent Validation Core} (Agents 0--3) whose scores are fused inside the primary Scene-Aware Agreement (SAA) loop and participate in game-theoretic analysis, and \textbf{two downstream Analytical Experts} (Agents 4--5) that consume real metadata from the Agent 0 fusion output while also contributing to the macro-uncertainty ($\sigma$) routing trigger. This role separation---validated in the ablation study (Section~\ref{sec:extended_eval})---prevents enrichment details from polluting the spatial-semantic consensus core, ensuring stable uncertainty-based routing.

The \textbf{Dual-LLM Coordinator} sits above all six agents. The \textbf{Primary VLM Coordinator} (LLaVA-OneVision) synthesizes all agent outputs into a structured JSON metadata record, resolving cross-modal contradictions between spatial detections (YOLOv11-seg), semantic VQA responses, and SigLIP scene classifications. A second \textbf{Critic LLM Auditor} independently reviews the primary decision, flagging hallucinations (e.g., when the VQA description mentions entities absent from spatial detections) and recording the macro-uncertainty score $\sigma$ used to trigger HITL triage.
...
\paragraph{Completed Implementation.} The pilot script \texttt{run\_second\_opinion_routing.py} was successfully extended and verified using the **Google Gemini** (Gemini 2.5 Flash) API. It supports querying multiple providers (\texttt{vllm}, \texttt{claude}, \texttt{gemini}) natively to run real-world pre-screening.
