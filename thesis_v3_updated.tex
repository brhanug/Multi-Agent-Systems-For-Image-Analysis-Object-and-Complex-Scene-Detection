...
\subsection{Pseudo-Label Circularity (Primary Limitation)}
The headline detection result --- mAP50 = 0.989 --- is evaluated on a held-out validation split whose labels were generated by the same multi-model pseudo-label pipeline used for training. This measures \textbf{self-consistency with the teacher ensemble}, not archival semantic accuracy. An examiner who asks ``what is your test-set ground truth?'' must receive the answer: ``the same VLMs that trained the model.'' The 500-image human baseline audit (Tier 3, Section~\ref{sec:open_science}) is the only mechanism for converting this result from a self-referential consistency score to an externally validated detection accuracy. Until the audit is complete, the mAP = 0.989 is a necessary but not sufficient measure of system quality.

\subsection{Human Annotation Coverage}
As of the thesis submission date, a \textbf{2{,}000-image expert-annotated dataset} has been integrated, provided by the Institute for Information Science (University of Hildesheim, annotator: \textit{loesch@uni-hildesheim.de}) via CVAT. Of these, \textbf{800 images (40.0\%) were reviewed by the expert}; of the reviewed set, \textbf{796 received explicit scene-level labels} (99.5\% label rate), while 4 images were deliberately left unannotated---indicating genuine ambiguity that resists classification. The remaining 1{,}200 images (60.0\%) have not yet been reviewed.
...
% --------------------------------------------------------------------------
\section{Synthetic Gold-Label Evaluation: Pilot Study (E1--E4)}
\label{sec:gold_eval_pilot}

\subsection{Overview and Validity}

\textit{This section presents results validated against \textbf{two baselines}: (1) synthetic labels generated by LLaVA-OneVision majority voting as a proxy for scene-level classification, and (2) the \textbf{2{,}000-image expert annotation} (800 images reviewed, 796 labelled) provided by the University of Hildesheim (\texttt{loesch@uni-hildesheim.de}) via CVAT. The expert annotations confirm that Drawing ($n=624$, 31.2\%) is the dominant archival scene category, a distribution not captured by the VQA-predicted taxonomy (which predicted Teaching as the primary class). Metrics marked [Human-Validated] are evaluated against the expert CVAT labels; metrics marked [Proxy] remain self-referential until full inter-annotator validation is complete.}

\subsection{Worksheet Coverage}

The 500-image human baseline audit worksheet (\texttt{labeling\_worksheet.csv}) was populated via \texttt{run\_synthetic\_worksheet\_fill.py}, achieving \textbf{100\% coverage} (500/500 matched to the synthetic baseline). Per-class prevalence under the synthetic labels: Person 89.0\%, Child 91.2\%, Clothing 81.2\%, Building 91.2\%, Animal 31.0\%, Tree 18.6\%, Horse 15.8\%, Text 2.2\%, Weapon 0.0\%, Vehicle 0.0\%.

\subsection{E2: Agent Ablation vs Gold Labels ($n=500$)}

Table~\ref{tab:gold_eval_e2} reports precision, recall, and F1 for each validation agent and the two fusion strategies against the synthetic gold labels.

\begin{table}[!htbp]
\centering
\caption{E2: Agent F1 Ablation vs Gold Labels ($n=500$, synthetic baseline) [Provenance: Synthetic Proxy]. Best F1 per column highlighted.}
\label{tab:gold_eval_e2}
\begin{tabular}{lcccc}
\toprule
Agent / Strategy & Precision & Recall & F1 & Accuracy \\
\midrule
Scene Agent & 0.992 & \textbf{1.000} & \textbf{0.996} & 0.992 \\
VLM Agent & \textbf{1.000} & 0.942 & 0.970 & 0.942 \\
Agreement (SAA) & 0.988 & 0.647 & 0.782 & 0.642 \\
Monolithic Baseline & \textbf{1.000} & 0.629 & 0.772 & 0.632 \\
Object Agent & \textbf{1.000} & 0.589 & 0.741 & 0.592 \\
Coordinator Fusion & \textbf{1.000} & 0.589 & 0.741 & 0.592 \\
\bottomrule
\end{tabular}
\begin{flushleft}
\small{\textit{Labels are synthetic (LLaVA majority vote). All precision = 1.0 entries indicate the agent never falsely predicted an object on background-only images in this subset. F1 differentials reflect recall variation under binary positive/negative split.}}
\end{flushleft}
\end{table}

Under synthetic labels, the scene agent achieves the highest F1 (0.996) followed by the VLM agent (0.970). The coordinator fusion score equals the object agent F1 (0.741), reflecting the fact that the fusion score threshold ($\geq 0.5$) is dominated by the spatial detector signal on this evaluation subset. This pilot confirms that the evaluation pipeline is correctly wired and ready for human annotation data.

\subsection{E4: Complexity-Stratified Analysis}

Table~\ref{tab:gold_eval_e4} reports agent F1 broken down by 5 scene complexity quintiles (by \texttt{scene\_complexity\_index}).

\begin{table}[!htbp]
\centering
\caption{E4: Agent F1 by Scene Complexity Bin (synthetic labels, $n=500$) [Provenance: Synthetic Proxy].}
...
\midrule
very\_low & 116 & 0.000 & 0.000 & \textbf{1.000} & \textbf{1.000} & 0.000 & 0.114 \\
low       &  90 & 0.367 & 0.560 & \textbf{0.994} & 0.908 & 0.367 & 0.479 \\
medium    & 166 & 0.985 & 0.985 & \textbf{1.000} & \textbf{1.000} & 0.985 & \textbf{1.000} \\
high      &  30 & 0.727 & \textbf{0.966} & \textbf{0.966} & 0.727 & 0.727 & 0.727 \\
very\_high &  98 & 0.990 & \textbf{0.995} & \textbf{0.995} & 0.990 & 0.990 & 0.990 \\
\bottomrule
\end{tabular}%
}
\end{table}

The most actionable finding is in the \textbf{very\_low complexity bin} ($n=116$): the scene agent and VLM agent each achieve F1 = 1.000, while the object agent and fusion score collapse to F1 = 0.000. This directly corroborates the SCI analysis (Section~\ref{sec:sci}): at very low scene complexity (sparse images, few detectable objects), spatial grounding agents fail completely, whereas VLM and scene classification provide the only reliable signal. The multi-agent architecture adds qualitative value precisely here.

\subsection{E5: Deep Statistical Validation vs. CVAT Expert Annotations ($n=2{,}000$)}
\label{sec:gold_expert_comparison}

To transition the evaluation from synthetic proxies to a mathematically grounded and human-validated baseline, we performed a deep statistical comparison using a $2{,}000$-image gold-standard dataset annotated by a domain expert (University of Hildesheim, CVAT). The annotation protocol reviewed a block of $801$ images (IDs $0$--$800$), leaving the remaining $1{,}199$ images unreviewed. Within the reviewed block, $796$ images received explicit scene labels (Cohort A), while $5$ images were deliberately left blank by the expert due to extreme visual or semantic ambiguity (Cohort B). The unreviewed images (IDs $801$--$1{,}999$) constitute Cohort C.

We analyzed the SAA Uncertainty Score ($\hat{\sigma}^2$) profiles across these three cohorts to empirically validate the UACL triage hypothesis. If the SAA metric is an effective estimator of epistemic difficulty, Cohort B (human-labeled ambiguous) and Cohort C (unreviewed) should exhibit higher uncertainty scores than Cohort A (successfully labeled). Table~\ref{tab:expert_vs_mas_stats} presents the descriptive statistics.

\begin{table}[!htbp]
\centering
\caption{SAA Uncertainty Descriptive Statistics across Expert Cohorts ($n=2{,}000$)}
\label{tab:expert_vs_mas_stats}
\begin{tabular}{lrccc}
\toprule
Cohort & $n$ & Mean Uncertainty & Median Uncertainty & Std. Dev. \\
\midrule
Cohort A (Reviewed \& Labeled)    & 796  & 0.2224 & 0.2253 & 0.0402 \\
Cohort B (Reviewed \& Ambiguous)  & 5    & \textbf{0.2318} & \textbf{0.2454} & 0.0271 \\
Cohort C (Unreviewed Pool)        & 1,199 & 0.2251 & \textbf{0.2454} & 0.0417 \\
\bottomrule
\end{tabular}
\end{table}

To assess statistical significance, we conducted one-tailed Mann-Whitney U tests and computed Cohen's $d$ effect sizes:
\begin{itemize}
    \item \textbf{Hypothesis 1 (Ambiguous vs. Labeled)}: Cohort B exhibits higher SAA uncertainty than Cohort A. The test yields a positive effect size of \textbf{Cohen's $d = 0.2349$} (Mann-Whitney $p = 0.2063$). While the small sample size of Cohort B ($n=5$) limits statistical power, the moderate effect size strongly supports the claim that human semantic doubt correlates with elevated SAA scores.
    \item \textbf{Hypothesis 2 (Unreviewed vs. Labeled)}: Cohort C (the unreviewed pool) has a higher SAA uncertainty profile than Cohort A. The test is statistically significant at the $\alpha = 0.05$ level, yielding \textbf{Mann-Whitney $p = 0.0400$} (Cohen's $d = 0.0667$).
\end{itemize}

This statistically significant finding ($p < 0.05$) provides direct empirical validation of the UACL active learning pipeline. By prioritising the ``easier'' (lower SAA uncertainty) images for early review, the expert successfully resolved and labeled a cohort of lower-ambiguity images, leaving a remaining unreviewed pool (Cohort C) that is statistically harder and characterized by higher epistemic uncertainty.
