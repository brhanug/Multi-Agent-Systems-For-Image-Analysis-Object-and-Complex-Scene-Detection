...
The architecture comprises a \textbf{four-agent Validation Core} (Agents 0--3) whose scores are fused inside the primary Scene-Aware Agreement (SAA) loop and participate in game-theoretic analysis, and \textbf{two downstream Analytical Experts} (Agents 4--5) that produce specialized demographic and geospatial metadata while also contributing to the macro-uncertainty ($\sigma$) routing trigger. This role separation --- validated in the ablation study (Section~\ref{sec:extended_eval}) --- prevents enrichment details from polluting the spatial-semantic consensus core, ensuring stable uncertainty-based routing.
...
\caption{Visual Historian-MAS Architecture. The foundational CV pipeline operates as Agent 0, extracting multimodal features. Five downstream analytical agents (Agents 1-5) provide independent historiographic reasoning. A central coordinator fuses all macro outputs, using ensemble uncertainty ($\sigma$) across the validation core and analytical agents to route ambiguous scenes to a Human-in-the-Loop review queue.}
...
\begin{itemize}
    ...
    \end{equation}
    \item \textbf{Monolithic Automated Triage}: Since standalone object detectors trained on modern visual concepts (like YOLOv11m) collapse under domain shift (achieving a real human mAP50 of only $0.1316$), relying on raw automated outputs is structurally unsafe for academic archives. This necessitates a 100\% manual audit, yielding zero labor savings.
