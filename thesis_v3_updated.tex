...
Detailed instructions and the project repository are available at: \url{https://github.com/brhanug/historical-image-analysis-thesis}

% --------------------------------------------------------------------------
\section{Latest Progress Update (April 2026)}

The evaluation pipeline was updated to align with supervisor guidance: treat the full workflow as a single baseline agent and compare it against specialized agents and the multi-agent coordinator.

\subsection{Implementation Progress}
\begin{itemize}
    \item Added a \textbf{monolithic baseline} (\texttt{monolithic\_pipeline\_agent}) representing the whole pipeline as one opaque decision score.
    \item Retained the \textbf{coordinator fusion} (\texttt{comparison\_fusion\_score}) as the disagreement-aware multi-agent strategy.
    \item Updated and re-ran the full evaluation stack:
    \begin{itemize}
        \item \texttt{scripts/core\_pipeline/run\_agent\_comparison.py}
        \item \texttt{scripts/core\_pipeline/run\_research\_evaluation.py}
        \item \texttt{scripts/core\_pipeline/run\_statistical\_report.py}
    \end{itemize}
    \item Refreshed all output artifacts under \texttt{results/multi\_agent/} (comparison, ablation, HITL efficiency, and statistical CI reports).
\end{itemize}

\subsection{Current Snapshot (12,110 Images)}
\begin{itemize}
    \item \textbf{Monolithic baseline mean}: 0.5867
    \item \textbf{Coordinator fusion mean}: 0.5632
    \item \textbf{Best single signal by mean}: \texttt{vlm\_agent} (0.8961)
    \item \textbf{HITL triage lift vs random}: 2.20x (top-k precision analysis)
    \item \textbf{Provenance state}: mixed (measured + proxy), explicitly tagged in exported reports.
\end{itemize}

This update operationalizes RQ1--RQ4 comparison framing and keeps all metrics reproducible through script-generated artifacts.

\subsection{RQ1--RQ4 Evaluation Summary (Latest Run)}
...
For RQ2 (disagreement as uncertainty signal), the current run indicates strong separation between prioritized and random review sets, while maintaining explicit mixed-provenance reporting (measured + proxy) to avoid over-claiming.
