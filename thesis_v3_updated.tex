...
We introduce \textit{Visual Historian-MAS}, a 6-agent coordinated architecture. We encapsulate the core computer vision pipeline---comprising restoration (CycleGAN, Real-ESRGAN, Stable Diffusion), object grounding (GroundingDINO, Florence-2, YOLOv11), scene classification (CLIP/SigLIP), and text extraction (Kosmos-2.5)---as a single foundational \textbf{Agent 0}. Surrounding this foundation are five independent analytical agents: a Temporal Historian (\textbf{Agent 1}), a Retrieval Agent (\textbf{Agent 2}), a Hallucination Critic (\textbf{Agent 3}), a Demographic Profiler (\textbf{Agent 4}), and a Geospatial Analyst (\textbf{Agent 5}). A central coordinator fuses their outputs via agreement- and uncertainty-aware scoring, routing ambiguous samples to human review.
