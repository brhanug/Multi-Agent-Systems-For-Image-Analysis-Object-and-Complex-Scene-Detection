...
As of the thesis submission date, a \textbf{2{,}000-image expert-annotated dataset} has been integrated, provided by the Institute for Information Science (University of Hildesheim, annotator: \textit{loesch@uni-hildesheim.de}) via CVAT. Of these, \textbf{796 images (39.8\%) received scene-level labels}, with the remaining 1{,}204 (60.2\%) left unannotated---reflecting genuine ambiguity in the archival material. The label distribution reveals that \textbf{Drawing (31.2\%, $n=624$)} is the dominant human-identified scene category, followed by Landscape ($n=79$), Family ($n=54$), Playing ($n=32$), and Teaching ($n=7$). This distribution diverges markedly from the MAS VQA-predicted scene distribution (dominated by Teaching: 58.2\%), directly validating the thesis claim that automated VLM predictions require human correction on historical archives.

The unannotated 60.2\% are precisely the ambiguous cases that the SAA triage mechanism is designed to flag: images where no expert consensus category was assigned. This empirically validates the \textbf{epistemic triage hypothesis}---that images left unlabelled by experts correspond to high MAS uncertainty regions.
